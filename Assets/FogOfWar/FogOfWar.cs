    ...
    void LateUpdate()
    {
        if (target != null)
        {
            ...
            if (Mathf.Abs(planeTopLeftRow - r) > 5 || Mathf.Abs(planeTopLeftCol - c) > 5)
            {
				...
				plane.transform.position = new Vector3(plane.transform.position.x, plane.transform.position.y + 3.0f, plane.transform.position.z);
                copyHeight();
            }

            copyData();

            plane.GetComponent<Renderer>().material.SetColor("_Color", tint);
            plane.GetComponent<Renderer>().material.SetFloat("_FadeTime", fadeTime);
            plane.GetComponent<Renderer>().material.SetFloat("_WorldTime", Time.time);
        }
    }
