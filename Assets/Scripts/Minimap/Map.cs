...
public class Map : MonoBehaviour
{
    ...
    public void WorldToMapCoordsStandard(Vector3 p, out float left, out float top)
    {
	...
	public void WorldToMapCoordsRotated(Vector3 p, out float left, out float top)
	{
		float xDist = Mathf.Clamp01((p.z - MapWorldTopLeft.x) / WorldMapVisibleSize);
		float zDist = Mathf.Clamp01((MapWorldTopLeft.z + p.x) / WorldMapVisibleSize);
    ...
    void CheckForPing()
    {
        if (pingEnabled && pingOnClick && Input.GetButtonDown(pingButtonName) && isVisible)
        {
            ...
            if (ScreenToMapCoords(mpos.x, mpos.y, out left, out top))
            {
				...
				Camera.main.transform.position = MapToWorldCoords(left, top);
            }
        }
    }
