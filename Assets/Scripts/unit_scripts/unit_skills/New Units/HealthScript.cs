...
public class HealthScript : MonoBehaviour {
	...
	void OnSkillResult(SkillResult skillresult) {
		if(skillresult.skillType == SkillResult.SkillType.Damage) {
			...
			CurrentHealth = Mathf.Clamp(CurrentHealth - (skillresult.skillPower - removedFromShield), 0.0f, BaseHealth);
		}

		else if(skillresult.skillType == SkillResult.SkillType.Heal) {
			CurrentHealth = Mathf.Clamp(CurrentHealth + skillresult.skillPower, 0.0f, BaseHealth);
		}
	...
	void regenerate(float healthRegen, int shieldRegen) {
		CurrentHealth = Mathf.Clamp(CurrentHealth + healthRegen, 0.0f, BaseHealth);
		CurrentShield = Mathf.Clamp(CurrentShield + shieldRegen, 0.0f, BaseShield);
	}
